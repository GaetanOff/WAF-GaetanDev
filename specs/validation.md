...
| 2026-06-11 | NFR-17 store no periodic stall | `go test ./... && go vet ./...` | pass | CleanupExpired no longer holds s.mu during the sweep (collect expired lock-free, delete per-key under short locks); GetVisitor is now lock-free (least-recently-set eviction); rate-limit buckets bounded by maxVisitors (evict least-recently-refilled). Tests: cleanup keeps live / removes expired; bucket cap evicts oldest. LRU test updated to set-based refresh |
| 2026-06-11 | FR-29 rich webhooks | `go test ./... && go vet ./...` | pass | Discord embeds (color by severity, emoji title, fields IP/path/method/action/country/score/reason, timestamp, footer req id) + Slack attachments; generic = enriched Alert JSON. Alerter interface now takes alert.Event (logger imports alert). Tests: Slack attachment delivered; Discord embed rich (color/fields/timestamp). Existing cooldown/retry tests migrated to Event |
| 2026-06-11 | FR-09 action classification | `go test ./... && go vet ./...` | pass | Removed actionFromStatus fallback (logger + metrics): upstream 5xx/4xx without X-WAF-Action are now action=PASS with real upstream_status, not BLOCK. Tests: upstream 502 → PASS + upstream_status 502 + no webhook alert; real X-WAF-Action=BLOCK still BLOCK + alerts |
| 2026-06-11 | FR-09 waf_latency_ms real | `go test ./... && go vet ./...` | pass | waf_latency_ms = total − upstream (was a duplicate of latency_ms). New internal/upstreamtime recorder fed by the proxy around the upstream call; logger subtracts it. Tests: recorder accumulates / nil-safe; middleware with 100ms total & 40ms upstream → waf_latency_ms=60, latency_ms=100 |
| 2026-06-19 | Slice 12.1 FR-39 under-attack | `go test ./...` | pass | Full suite green incl. new tests: antiddos hysteresis (enter/exit/cooldown), Observe trigger + shadow, per-domain isolation, global scope, LRU eviction, transition observer, middleware headers (X-WAF-Under-Attack/-Enforce, shadow) ; challenge forcing (raw GET challenged under attack, JSON exempt, valid cookie bypass, non-GET ignored) ; config validation (scope/pressure/order/cooldown/max_domains) |
| 2026-06-19 | Slice 12.1 FR-39 under-attack | `go vet ./...` | pass | No vet findings |
| 2026-06-19 | Slice 12.1 FR-39 under-attack | `go build ./...` | pass | New file internal/middleware/antiddos/underattack.go ; wired in cmd/waf/main.go (transition observer → metric + alert) |
| 2026-06-19 | Slice 12.1 FR-39 under-attack | `gofmt -l internal/ cmd/` | pass | No formatting diffs |
...
| 2026-09-01 | FR-30 assainissement ingress | `go build ./... && go vet ./... && go test ./...` | pass | 433 tests, 42 paquets (413 / 41 avant). Nouveau paquet `internal/middleware/ingress`, câblé en position la plus externe de `routes()` |
| 2026-09-01 | FR-30 reproduction | `go test ./cmd/waf/` (avant correctif) | **faille reproduite** | Visiteur sous le seuil, `Accept: text/html`, `X-WAF-Action: PASS` → **204 de l'upstream** au lieu de la page de challenge : le challenge, le rate limiting, l'intégrité, le threat intel et le moteur de règles sont tous court-circuités par ce seul en-tête |
| 2026-09-01 | FR-30 mutation check | `go test ./cmd/waf/ -run TestRoutesIgnoresClientSuppliedWAFHeaders` | pass | **Vérifié en échec** : câblage retiré de `routes()` → les cas `action_PASS` et `action_PASS_en_minuscules` atteignent l'upstream (204). Le cas témoin (sans en-tête forgé) garantit que le test n'est pas vide |
| 2026-09-01 | FR-30 mutation check | `go test ./internal/middleware/ingress/` | pass | **Vérifié en échec** : filtre remis en `strings.HasPrefix(name, "X-Waf-")` (sensible à la casse, dépendant de la canonicalisation `net/http`) → `TestMiddlewareStripsNonCanonicalHeaderKeys` échoue sur `x-waf-action`, `X-WAF-SCORE`, `x-WaF-Risk-Geo` |
| 2026-09-01 | FR-30 mutation check | `go test ./cmd/waf/ -run TestRoutesRiskEngineShadowByDefault` | pass | **Vérifié en échec** : détecteur de familles vidé → `X-WAF-Risk-Decision = "ALLOW"`. Le test n'asserte plus seulement « le proxy est appelé » (vrai trivialement sans signal de risque) mais que le moteur a bien rendu une décision de mitigation que le shadow a supprimée. Décision observée : `THROTTLE`, score fusionné 58 — l'ancien commentaire « sans shadow, ce serait un BLOCK » était inexact |
| 2026-09-01 | FR-30 couverture | `go test ./internal/middleware/ingress/ -cover` | pass | 100.0% des instructions |
| 2026-09-01 | FR-30 formatage | `gofmt -l` sur copies normalisées LF des 4 fichiers touchés | pass | Aucun diff. `gofmt -l` brut signale ~27 fichiers du dépôt : artefact CRLF du poste Windows (`core.autocrlf=true`), les blobs commités restent en LF — sans rapport avec ce diff |
| 2026-09-01 | FR-30 lint | `golangci-lint run ./internal/middleware/ingress/... ./cmd/waf/...` | pass | Aucun finding hors le bruit `gofmt`/CRLF ci-dessus |
| 2026-09-01 | FR-30 race detector | `go test -race ./internal/middleware/ingress/ ./cmd/waf/` | **non exécuté localement** | `-race` exige CGO et aucun compilateur C n'est installé sur le poste. Le middleware ne porte aucun état partagé — il ne mute que le `r.Header` de la requête courante. Couvert par le job `test` du CI (ubuntu-latest, `go test ./... -race`) |
| 2026-09-01 | FR-19 × FR-30 régression | `go test ./cmd/waf/` (avant correctif) | **régression reproduite** | L'assainissement livré en T14.1 supprimait le `X-WAF-Origin-Token` que l'upstream retransmet à `GET /waf/origin/verify` : **401 sur un token valide**, la vérification FR-19 devenait inopérante |
| 2026-09-01 | FR-19 × FR-30 correctif | `go build ./... && go vet ./... && go test ./...` | pass | 440 tests, 42 paquets. `origin.CaptureInboundToken` monté à l'extérieur de l'assainisseur ; `VerifyHandler` lit la valeur hors de `r.Header` |
| 2026-09-01 | FR-19 mutation check | `go test ./cmd/waf/ -run TestRoutesOriginVerifyReadsTheRetransmittedToken` | pass | **Vérifié en échec** : capture retirée de `routes()` → le cas `token_valide` retombe à 401 `{"valid":false}` |
| 2026-09-01 | FR-19 couverture | `go test ./internal/origin/ -cover` | pass | 97,0% des instructions |
| 2026-09-01 | FR-17 `X-Real-IP` reproduction | `go test ./internal/rules/` (avant correctif) | **faille reproduite** | `X-Real-IP: 8.8.8.8` faisait échapper un client réellement en `10.1.2.3` à une règle `ip in_cidr ["10.0.0.0/8"]` de blocage ; `X-Real-IP: 10.1.2.3` faisait matcher un client réellement en `8.8.8.8`. Exploitable **à travers Cloudflare**, qui ne réécrit pas cet en-tête |
| 2026-09-01 | FR-17 correctif | `go build ./... && go vet ./... && go test ./...` | pass | 447 tests, 42 paquets. `clientIP` délègue à `cloudflare.RealIP` : un seul chemin d'IP pour les 21 consommateurs du dépôt |
...
- **Implémenté** : compteur de pression par domaine (anneau de buckets + éviction LRU `max_tracked_domains`) ; contrôleur de mode sous attaque avec hystérésis (entrée `trigger_pressure`, sortie sous `exit_pressure` soutenu `cooldown`) ; headers internes `X-WAF-Under-Attack` (journalisé) et `X-WAF-Under-Attack-Enforce` (forçage) ; challenge forcé pour les requêtes sans clearance (cookie/whitelist/bot vérifié déjà PASS) avec exemption des clients JSON/non-GET ; mode `shadow` (état journalisé sans enforcement) ; champ de log `under_attack` (contrat security-event.schema.json étendu) ; métriques `waf_under_attack{domain}` + `waf_under_attack_challenges_total{domain}` ; alerte FR-29 entrée/sortie (`under_attack_start`/`under_attack_end`, sévérités/titres dédiés).
- **Activation par défaut** : `antiddos.under_attack.enabled: true` est ajouté aux défauts (`config.Default()`). Les clés absentes d'un `config.yml` existant conservent ces défauts (YAML v3 n'écrase que les clés présentes) : le mode s'active au prochain déploiement. Pour calibrer sans risque, passer `shadow: true` ≥ 24 h (NFR-15) avant enforcement.
...
### FR-30 — assainissement des en-têtes `X-WAF-*` : périmètre du correctif

- **Corrigé** : les middlewares se coordonnent en posant des en-têtes `X-WAF-*` **sur la requête**, relus en aval — 16 lectures `r.Header.Get`. Rien ne les nettoyait à l'entrée du pipeline, donc un client pouvait les fabriquer. `X-WAF-Action: PASS` est testé en court-circuit par cinq middlewares : un seul en-tête suffisait à traverser tout le WAF. Le nouveau paquet `internal/middleware/ingress` supprime le préfixe entier, en position la plus externe de `routes()`.
- **Pourquoi un préfixe et non une liste nominative** : une liste doit être tenue à jour à chaque nouvel en-tête interne. Son oubli ne casse rien de visible — il rouvre silencieusement le contournement. Le préfixe est correct par construction.
- **Pourquoi `EqualFold` et non `HasPrefix` sur la forme canonique** : `net/http` canonicalise les clés issues du fil, donc `HasPrefix(name, "X-Waf-")` suffirait *aujourd'hui*. Faire reposer une frontière de sécurité sur un invariant interne de la bibliothèque est un pari inutile : la comparaison insensible à la casse coûte un parcours de quelques en-têtes par requête.
- **Ce qui n'est pas touché** : `CF-Connecting-IP` (FR-02), les `X-Forwarded-*`, `Authorization` (FR-10), et tout en-tête hors préfixe — y compris `X-WAF` nu et un voisin comme `X-WAFER-*`, couverts par un test dédié. Les signaux posés **à l'intérieur** du pipeline (PASS de la whitelist FR-04 et du bypass d'assets FR-24, `X-WAF-Risk-*` des détecteurs, `X-WAF-Origin-Token` FR-19) restent intacts : l'assainissement s'applique une seule fois, à l'entrée.
- **Effet de bord sur les tests existants** : deux tests de `cmd/waf` utilisaient la faille comme point d'injection (`X-WAF-Risk-*` posés sur la requête cliente avec `detectors = nil`). `TestRoutesAppliesRiskDecisionBeforeProxy` échouait franchement ; `TestRoutesRiskEngineShadowByDefault` passait pour la mauvaise raison — il vérifie que le proxy *est* appelé, ce qui devenait vrai trivialement. Les deux injectent désormais via un détecteur du paramètre `detectors`, comme en production.
- **Hors périmètre, délibérément** : l'API admin (port 9090) ne lit aucun `X-WAF-*` — vérifié — et n'a donc pas besoin du middleware ; les `r.Trailer` ne sont lus par aucun middleware ; l'audit des autres surfaces de confiance implicite (`X-Forwarded-*` hors chemin Cloudflare, `Host` en amont du routage `domains[]`) demande ses propres specs. Le `Request Processing Pipeline` d'`architecture.md` reste par ailleurs **en retard sur le code** (ni `maintenance`, ni `secheaders`, ni `staticassets`, ni moteur de risque n'y figurent) : seule l'étape `[0]` a été ajoutée, une remise à niveau complète du diagramme sort du périmètre de ce correctif.

### FR-19 × FR-30 — pourquoi une capture et non une exemption de chemin

- **La régression** : `GET /waf/origin/verify` est un oracle appelé **par l'upstream**, qui lui retransmet le `X-WAF-Origin-Token` reçu. L'assainissement d'ingress (T14.1) supprimait cette valeur avant le handler : l'endpoint répondait 401 à tout token, valide compris. FR-19 spécifie explicitement cet endpoint — la règle FR-30 telle qu'écrite en v3.2.0 le rendait inopérant.
- **Exemption par chemin écartée** : tester `r.URL.Path == "/waf/origin/verify"` dans l'assainisseur aurait été plus court, mais sa correction dépendrait alors d'une normalisation de chemin identique à celle du routeur (`//waf/origin/verify`, `/waf/origin/verify/../..`, encodages). Un désaccord entre les deux normalisations est un vecteur de contournement classique — et ici il rendrait l'assainissement sautable sur un chemin arbitraire.
- **Capture retenue** : `origin.CaptureInboundToken` mémorise la valeur dans le contexte de requête, en amont de l'assainisseur, et `VerifyHandler` la lit hors de `r.Header`. Le middleware ne connaît aucun chemin, ne réinjecte rien, et n'interprète pas la valeur — la vérification HMAC reste seule juge. Le repli sur l'en-tête garde le handler utilisable en test unitaire sans masquer l'absence de capture : sans capture, l'en-tête est déjà vide.
- **Périmètre de l'exception** : un seul en-tête, un seul endpoint. `ingress.Middleware` cesse d'être « le middleware le plus externe » et devient « celui qui précède tout middleware lisant ces en-têtes » — la capture est la seule chose autorisée en amont, et elle ne fait que mémoriser.

### T14.3 — audit des surfaces de confiance implicite : ce qui a été corrigé et ce qui attend une décision

- **Inventaire** : toutes les lectures d'en-tête entrant servant à une décision de sécurité, hors `X-WAF-*` (couvert par T14.1). Résultat : une seule incohérence franche, deux questions de politique.
- **Corrigé (FR-17)** : `internal/rules/rules.go` résolvait la condition `ip` via `X-Real-IP`. C'était le **seul** des 21 consommateurs d'IP du dépôt à ne pas passer par `cloudflare.RealIP` — la whitelist, la blacklist, le rate limit, le trust score, l'anti-bot, l'anti-DDoS, le threat intel, le challenge et le moteur de risque le font tous. La confusion venait du sens de l'en-tête : `X-Real-IP` est **posé par le proxy vers l'upstream**, sa présence en entrée n'a aucune signification.
